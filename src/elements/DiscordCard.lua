...
local function GetInviteCode(Value)
	local Text = Trim(Value)
	local Code = string.match(Text, "discord%.gg/([^/]+)") or string.match(Text, "discord%.com/invite/([^/]+)")
	if not Code then
		Code = Text
	end
	return Code
...
local function LoadDiscordIcon(Invite, Callback)
	local Code = GetInviteCode(Invite)
	if not Code or Code == "" then return end

	task.spawn(function()
		local HttpService = game:GetService("HttpService")
		local Success, Response = pcall(function()
			return game:HttpGet("https://discord.com/api/v10/invites/" .. Code)
		end)
		...
		if not DataSuccess or not Data or not Data.guild then return end

		local GuildId = Data.guild.id
		local IconHash = Data.guild.icon

		if GuildId and IconHash then
			local Extension = string.find(IconHash, "a_") == 1 and "gif" or "png"
			local IconUrl = string.format("https://cdn.discordapp.com/icons/%s/%s.%s?size=128", GuildId, IconHash, Extension)
			
			local ImageSuccess, ImageContent = pcall(function()
				return game:HttpGet(IconUrl)
			end)

			if ImageSuccess and ImageContent then
				local FileName = string.format("DiscordIcon_%s.%s", GuildId, Extension)
				local FilePath = "WindUI/Cache/" .. FileName

				if not isfolder("WindUI") then makefolder("WindUI") end
				if not isfolder("WindUI/Cache") then makefolder("WindUI/Cache") end

				writefile(FilePath, ImageContent)
...
function Element:New(Config)
	...
	LoadDiscordIcon(DiscordCard.Invite, function(AssetId)
		if IconBubble:FindFirstChild("ServerIcon") then
			if HeaderIcon then
				HeaderIcon.Visible = false
			end
			IconBubble.ImageTransparency = 1
			ServerIcon.Image = AssetId
